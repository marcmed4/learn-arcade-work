...
class MyGame(arcade.Window):
    ...
    def on_mouse_press(self, x, y, button, modifiers):
        ...
        [row.count(1) for row in self.grid]
        total_count = sum(row.count(1) for row in self.grid)
        print("There are a total of", total_count, "cells selected.")
        ...
        for row in range(ROW_COUNT):
            ...
            if continuous_count > 2:
                print("There are", continuous_count, "consecutive blocks selected on row", row, ".")
                continuous_count = 0
        ...
        for column in range(COLUMN_COUNT):
            ...
            for row in range(ROW_COUNT):
                if self.grid[row][column] == 1:
                    ...
                    continuous_count += 1
            if continuous_count > 2:
                print("There are", continuous_count, "consecutive blocks selected on column", column, ".")
                continuous_count = 0
